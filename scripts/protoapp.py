...
class aplicacion(tk.Frame):
    ...
    def ruta(self):
        ...
        if type(self.ROCE) != list:
            ...
            for val in br:
                val.destroy()
            l2 = tk.Label(self, text="Selecciona tu entidad ")
            l2.pack()
            edos = ttk.Combobox(self,
                state='readonly',
                textvariable=self.estado,
                values=list(self.historial['Entidad'].unique())+['Todos']
                )
            edos.pack()                    
            b2=tk.Button(self,
                      text ="Seleccionar",
    ...
    def retrasos(self):
        ...
        if edo == '' or edo== 'vacio':
            ...
            return
        if edo != 'Todos':
            filtro = self.historial.loc[self.historial['Entidad']==edo]
        ...
        if edo != 'Todos':
            OA1 = self.OA.loc[self.OA['Entidad']==edo]
